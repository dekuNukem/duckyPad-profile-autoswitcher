...
connection_info_label.place(x=scaled_size(PADDING+110), y=scaled_size(5))
...
connection_button.place(x=scaled_size(PADDING), y=scaled_size(5))
...
prev_profile_button.place(x=scaled_size(PADDING+400), y=scaled_size(5))
...
next_profile_button.place(x=scaled_size(PADDING+500), y=scaled_size(5))
...
user_manual_button.place(x=scaled_size(PADDING), y=scaled_size(5))

discord_button = Button(dashboard_lf, text="Discord", command=open_discord)
discord_button.config(width=11, height=1)
discord_button.place(x=scaled_size(PADDING+100), y=scaled_size(5))

discord_button = Button(dashboard_lf, text="Backup", command=open_save_folder)
discord_button.config(width=11, height=1)
discord_button.place(x=scaled_size(PADDING+200), y=scaled_size(5))
...
autoswitch_status_label.place(x=scaled_size(PADDING), y=scaled_size(40))
...
def create_rule_window(existing_rule=None):
    ...
    rule_window.geometry(f"{scaled_size(640)}x{scaled_size(510)}")
    ...
    rule_edit_lf.place(x=scaled_size(PADDING), y=scaled_size(5))

    app_name_label = Label(master=rule_window, text="IF app name contains:")
    app_name_label.place(x=scaled_size(PADDING+10), y=scaled_size(25))
    app_name_entrybox = Entry(rule_window)
    app_name_entrybox.place(x=scaled_size(PADDING+240), y=scaled_size(25), width=scaled_size(200))
    
    window_name_label = Label(master=rule_window, text="AND window title contains:")
    window_name_label.place(x=scaled_size(PADDING+10), y=scaled_size(50))
    window_name_entrybox = Entry(rule_window)
    window_name_entrybox.place(x=scaled_size(PADDING+240), y=scaled_size(50), width=scaled_size(200))

    switch_to_label = Label(master=rule_window, text="THEN jump to profile (Name or Number):")
    switch_to_label.place(x=scaled_size(PADDING+10), y=scaled_size(75))
    switch_to_entrybox = Entry(rule_window)
    switch_to_entrybox.place(x=scaled_size(PADDING+240), y=scaled_size(75), width=scaled_size(200))
    ...
    window_list_fresh_button.config(width=scaled_size(72), height=1)
    window_list_fresh_button.place(x=scaled_size(PADDING), y=scaled_size(220))
    windows_list_text_area = ScrolledText.ScrolledText(window_list_lf, wrap='none', width = scaled_size(73), height = 13)
    windows_list_text_area.place(x=scaled_size(PADDING), y=scaled_size(5))
    root.update()
